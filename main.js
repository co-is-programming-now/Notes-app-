...
const textArea = document.querySelector(".note").addEventListener("change", correctionService);
...
getNotes().forEach((note) => {
  const getBackNote = createNote(note.id, note.valueNote);
  // quiero ver si aqui podria incluir algo como 
  // viste que el primer nota que tengo no tiene que estar bueno
  // al iniciar quiero q si el array de notas esta vacion
  // significa que es nuevo entonces y solo entonces quiero que se presente
  // un mensaje que diga las instrucciones
  // y se sale cuando el usuario acepta y pone okei
  // y tambien estaria bueno que el usuario tubiera algo asi como un video tutorial
  // patente pendiente 
});
